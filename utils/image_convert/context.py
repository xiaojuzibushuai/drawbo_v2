...
class GCodeContext:
    ...
    def generate(self,outputPath:str):
      ...
      if self.continuous == 'true':
        ...
        for codeset in codesets:
          for line in codeset:
            # print(line)
            self.write_to_file(outputPath,line)
      else:
    ...
    def write_to_file(self, file_name: str,line: str):
        with open(file_name, 'a+') as file:
            file.write(line+"\n")
    
    def start(self):
      self.codes.append("M300 S%d (pen down)" % self.pen_down_angle)
      self.codes.append("G4 P%d (wait %dms)" % (self.start_delay, self.start_delay))
      self.drawing = True

    def stop(self):
      self.codes.append("M300 S%d (pen up)" % self.pen_up_angle)
    ...
    def draw_to_point(self, x, y, stop=False):
      ...
      if stop:
        ...
        if self.drawing == False:
            self.codes.append("M300 S%d (pen down)" % self.pen_up_angle)
            self.codes.append("G4 P%d (wait %dms)" % (self.start_delay, self.start_delay))
            self.drawing = True

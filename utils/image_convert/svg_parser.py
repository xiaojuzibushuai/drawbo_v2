...
from utils.image_convert import cspsubdiv, inkex, simplepath, entities, cubicsuperpath
from utils.image_convert.bezmisc import beziersplitatt
from utils.image_convert.simpletransform import composeTransform, parseTransform, applyTransformToPath
...
class SvgLayerChange():
  ...
  def get_gcode(self,context):
    context.codes.append("%s" % self.layer_name)
...
class SvgParser:
  ...
  def parse(self):
    ...
    self.svgWidth = self.getLength('width', 354) * 0.25
    self.svgHeight = self.getLength('height', 354) * 0.25
    self.recursivelyTraverseSvg(self.svg, [[0.25, 0.0, -(self.svgWidth/2.0)], [0.0, -0.25, (self.svgHeight/2.0)]])
  ...
  def recursivelyTraverseSvg(self, nodeList, 
    ...
    for node in nodeList:
      ...
      # matNew = composeTransform(matNew, parseTransform("rotate(90)"))

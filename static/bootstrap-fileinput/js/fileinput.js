...
(function (factory) {
    ...
    FileInput.prototype = {
        ...
        _parseError: function (operation, jqXHR, errorThrown, fileName) {
            ...
            self.cancelling = false;
            return fileName ? '<d>' + fileName + ': </d>' + errMsg : errMsg;
        },
    ...
    $.fn.fileinputLocales.en = {
        ...
        msgSizeTooSmall: 'File "{name}" (<d>{size} KB</d>) is too small and must be larger than <d>{minSize} KB</d>.',
        msgSizeTooLarge: 'File "{name}" (<d>{size} KB</d>) exceeds maximum allowed upload size of <d>{maxSize} KB</d>.',
        msgFilesTooLess: 'You must select at least <d>{n}</d> {files} to upload.',
        msgFilesTooMany: 'Number of files selected for upload <d>({n})</d> exceeds maximum allowed limit of <d>{m}</d>.',
        msgTotalFilesTooMany: 'You can upload a maximum of <d>{m}</d> files (<d>{n}</d> files detected).',
        ...
        msgResumableUploadRetriesExceeded: 'Upload aborted beyond <d>{max}</d> retries for file <d>{file}</d>! Error Details: <pre>{error}</pre>',
